...
$formguests = $_POST['formguests'];
$formmonth = $_POST['formmonth'];
$formday = $_POST['formday'];
$formhour = $_POST['formhour'];
$formminutes = $_POST['formminutes'];

if (isset($_POST['formampm'])) {
    $formampm = $_POST['formampm'];
}else{
	$formampm = "";
}
...
$to = $emailto;
$sub = __('A new reservation from ','eatery'). $sitename;
...
if ($formemail > ""){
	$email_from = filter_var($formemail, FILTER_SANITIZE_EMAIL);
}else{
	$email_from = $emailto;
}

if ($formname > ""){
	$email_from_name = filter_var($formname, FILTER_SANITIZE_STRING);
}else{
